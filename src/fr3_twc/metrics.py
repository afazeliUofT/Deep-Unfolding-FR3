...
def tone_grouping_error(
    ...
    den = tf.sqrt(
        tf.reduce_sum(tf.abs(h0) ** 2, axis=[2, 3, 4]) * tf.reduce_sum(tf.abs(h1) ** 2, axis=[2, 3, 4])
        + tf.cast(1e-15, tf.float32)
    )
    # Use |num|/den instead of complex division to avoid complex/real dtype mismatch.
    corr_adj = tf.abs(num) / tf.maximum(den, tf.cast(1e-15, den.dtype))
    rho_adj = tf.reduce_mean(corr_adj).numpy()
    ...
    for g in group_sizes:
        ...
        nmse = tf.reduce_mean(tf.abs(h - h_hat) ** 2) / tf.maximum(
            tf.reduce_mean(tf.abs(h) ** 2), tf.cast(1e-15, tf.float32)
        )
