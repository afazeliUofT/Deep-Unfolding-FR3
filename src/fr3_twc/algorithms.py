...
def project_per_bs_power(w: tf.Tensor, p_tot_watt: float) -> tf.Tensor:
    ...
    return w * tf.cast(scale[:, None, :, None, None], w.dtype)
...
def initial_mf_precoder(batch: WidebandBatch, use_gate: bool = True) -> tf.Tensor:
    ...
    for bs_idx in range(b):
        ...
        w_b = w_b / tf.cast(norm, w_b.dtype)
        w_parts.append(w_b)

    w = tf.stack(w_parts, axis=2)  # [S,K,B,M,U_b]
    if use_gate:
        w = w * tf.cast(batch.static_gate[:, :, None, None, None], w.dtype)
...
def fixed_pgd(
    ...
    for _ in range(int(steps)):
        ...
        w_new = w - tf.cast(step_size, w.dtype) * grad
        if use_gate:
            w_new = w_new * tf.cast(batch.static_gate[:, :, None, None, None], w.dtype)
        w_new = project_per_bs_power(w_new, batch.bs_power_budget_watt)
        w = tf.cast(damping, w.dtype) * w_new + tf.cast(1.0 - damping, w.dtype) * w
