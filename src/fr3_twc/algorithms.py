...
def _equal_count_group_masks(cost_k: tf.Tensor, safe_mask: tf.Tensor, num_groups: int) -> List[tf.Tensor]:
    ...
    for g in range(num_groups):
        lo_rank = tf.cast(tf.floor(tf.cast(safe_counts, tf.float32) * (float(g) / float(num_groups))), tf.int32)
        hi_rank = tf.cast(tf.ceil(tf.cast(safe_counts, tf.float32) * (float(g + 1) / float(num_groups))), tf.int32)
