...
def soft_outage_surrogate(excess: tf.Tensor, beta: float = 10.0, slack: float = 0.0) -> tf.Tensor:
    """Boundary-aware smooth outage surrogate on the worst FS excess per sample.

    Unlike a raw sigmoid, this surrogate is exactly zero at the feasibility boundary
    (max_excess == slack) and stays near zero for slightly negative excess values.
    This avoids penalizing repaired/recovered feasible points as if they were outages.
    """
    real_dtype = excess.dtype
    max_excess = tf.reduce_max(excess, axis=1) - tf.cast(slack, real_dtype)
    beta_t = tf.cast(beta, real_dtype)
    zero_level = tf.math.log(tf.cast(2.0, real_dtype)) / beta_t
    smooth_pos = tf.nn.softplus(beta_t * max_excess) / beta_t - zero_level
    smooth_pos = tf.nn.relu(smooth_pos)
    return tf.reduce_mean(smooth_pos)
...
def _equal_count_group_masks(cost_k: tf.Tensor, safe_mask: tf.Tensor, num_groups: int) -> List[tf.Tensor]:
    """Split the safe subbands into low-cost -> high-cost groups with roughly equal counts."""
    num_groups = max(1, int(num_groups))
    if num_groups == 1:
        return [safe_mask]

    num_samples = tf.shape(cost_k)[0]
    num_subbands = tf.shape(cost_k)[1]
    huge = tf.cast(1e12, cost_k.dtype)
    ranked_cost = tf.where(safe_mask, cost_k, huge)
    sorted_idx = tf.argsort(ranked_cost, axis=1, direction="ASCENDING")
    safe_counts = tf.reduce_sum(tf.cast(safe_mask, tf.int32), axis=1)

    batch_ids = tf.tile(tf.range(num_samples)[:, None], [1, num_subbands])
    gather_nd = tf.stack([batch_ids, sorted_idx], axis=-1)
    pos = tf.tile(tf.range(num_subbands)[None, :], [num_samples, 1])

    groups: List[tf.Tensor] = []
    scatter_shape = tf.shape(cost_k)
    for g in range(num_groups):
        lo_rank = tf.cast(tf.math.floor(tf.cast(safe_counts, tf.float32) * (float(g) / float(num_groups))), tf.int32)
        hi_rank = tf.cast(tf.math.ceil(tf.cast(safe_counts, tf.float32) * (float(g + 1) / float(num_groups))), tf.int32)
        hi_rank = tf.maximum(hi_rank, lo_rank)
        sel = tf.logical_and(pos >= lo_rank[:, None], pos < hi_rank[:, None])
        sel = tf.logical_and(sel, pos < safe_counts[:, None])
        idx = tf.boolean_mask(gather_nd, sel)
        updates = tf.ones([tf.shape(idx)[0]], dtype=tf.bool)
        group_mask = tf.scatter_nd(idx, updates, scatter_shape)
        group_mask = tf.logical_and(group_mask, safe_mask)
...
def recover_rate_with_feasible_mask(
    ...
    """Recover rate from a feasible point by progressively re-opening the safest subbands."""
    ...
    num_groups = int(twc_get(cfg, ('algorithm', 'recovery_num_groups'), 4))
    ...
    num_samples = tf.shape(cost_k)[0]
    num_subbands = tf.shape(cost_k)[1]
    min_k = tf.maximum(1, tf.cast(tf.round(tf.cast(num_subbands, tf.float32) * min_safe_frac), tf.int32))
    rank_mask = tf.tile(tf.sequence_mask(min_k, maxlen=num_subbands)[None, :], [num_samples, 1])
    sorted_idx = tf.argsort(cost_k, axis=1, direction='ASCENDING')
    scatter_shape = tf.shape(cost_k)
    batch_ids = tf.tile(tf.range(num_samples)[:, None], [1, num_subbands])
    gather_nd = tf.stack([batch_ids, sorted_idx], axis=-1)
    selected_indices = tf.boolean_mask(gather_nd, rank_mask)
    selected_updates = tf.ones(tf.shape(selected_indices)[0], dtype=tf.bool)
    min_safe_mask = tf.scatter_nd(selected_indices, selected_updates, scatter_shape)
    safe_mask = tf.logical_or(safe_mask, min_safe_mask)

    target = tf.cast(margin, real_dtype) * i_max
    best = w_feasible
    eta_values: List[tf.Tensor] = []
    active_values: List[tf.Tensor] = []

    for group_mask in _equal_count_group_masks(cost_k, safe_mask, num_groups):
        active = tf.reduce_any(group_mask, axis=1)
        mask = tf.cast(group_mask, real_dtype)[:, :, None, None, None]
        delta = tf.complex(mask * tf.math.real(w_reference - best), mask * tf.math.imag(w_reference - best))

        lo = tf.zeros([num_samples], dtype=real_dtype)
        hi = tf.where(active, tf.ones([num_samples], dtype=real_dtype), tf.zeros([num_samples], dtype=real_dtype))
        best_group = best
        for _ in range(max(1, bisect_iters)):
            mid = (lo + hi) / tf.cast(2.0, real_dtype)
            w_try = best + _scale_complex_by_real(delta, mid[:, None, None, None, None])
            w_try = project_per_bs_power(w_try, batch.bs_power_budget_watt)
            i_try = fs_interference(batch, w_try)
            feasible = tf.logical_and(active, tf.reduce_all(i_try <= target, axis=1))
            best_group = tf.where(feasible[:, None, None, None, None], w_try, best_group)
            lo = tf.where(feasible, mid, lo)
            hi = tf.where(feasible, hi, mid)
        best = best_group
        eta_values.append(lo)
        active_values.append(tf.cast(active, real_dtype))

    eta_stack = tf.stack(eta_values, axis=1) if eta_values else tf.zeros([num_samples, 1], dtype=real_dtype)
    active_stack = tf.stack(active_values, axis=1) if active_values else tf.zeros([num_samples, 1], dtype=real_dtype)
    eta_den = tf.maximum(tf.reduce_sum(active_stack), tf.cast(1.0, real_dtype))
    eta_mean = tf.reduce_sum(eta_stack * active_stack) / eta_den
    eta_min = tf.reduce_min(tf.where(active_stack > 0, eta_stack, tf.ones_like(eta_stack)))
    ...
    stats = {
        ...
        'recovery_eta_mean': float(eta_mean.numpy()),
        'recovery_eta_min': float(eta_min.numpy()),
        'recovery_num_groups': float(num_groups),
