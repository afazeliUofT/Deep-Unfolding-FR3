...
def train_unfolded_model(cfg: Any) -> Tuple[tf.keras.Model, tf.keras.optimizers.Optimizer]:
    ...
    if mode in {"budget_aware_primal_dual", "primal_dual_budgeted", "primal_dual"}:
...
def unfolded_inference(model: tf.keras.Model, batch: WidebandBatch) -> AlgorithmResult:
    start = time.perf_counter()
    w, hist = model(batch=batch, training=False)
    runtime_s = time.perf_counter() - start
    algo_name = getattr(model, "algorithm_name", "scenario_adaptive_unfolded")
    extra = {}
