...
def unfolded_inference(
    ...
    do_repair = repair and isinstance(model, FeasibilityRestoredPrimalDualUnfolded)
    if do_repair:
        lam0 = tf.cast(last_lambda, batch.delay_spread_ns.dtype) if last_lambda is not None else initial_dual_from_batch(batch)
        w, lam1, cal_hist = dual_calibration_refine(batch, w, lam0, model.cfg, use_gate=True)
        hist = _append_hist(hist, cal_hist)
        w, repair_stats = repair_fs_feasibility(batch, w, model.cfg, lam=lam1)
