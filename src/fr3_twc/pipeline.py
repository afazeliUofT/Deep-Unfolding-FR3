...
def _teacher_for_mode(
    ...
    if mode in RR_MODES:
        use_budget_early = bool(twc_get(cfg, ("teacher", "rr_use_budget_teacher_before_switch"), True))
        default_switch = max(1, int((total_epochs or 1) * 0.33))
        switch_epoch = int(twc_get(cfg, ("teacher", "rr_switch_epoch"), default_switch))
        if use_budget_early and epoch is not None and epoch < switch_epoch:
            return solve_budget_dual_teacher(cfg, batch)
        return solve_repair_recover_teacher(cfg, batch)
    if mode in FR_MODES:
        use_budget_early = bool(twc_get(cfg, ("teacher", "fr_use_budget_teacher_before_switch"), True))
        default_switch = max(1, int((total_epochs or 1) * 0.33))
        switch_epoch = int(twc_get(cfg, ("teacher", "fr_switch_epoch"), default_switch))
        if use_budget_early and epoch is not None and epoch < switch_epoch:
            return solve_budget_dual_teacher(cfg, batch)
    return solve_budget_dual_teacher(cfg, batch)


def _use_postprocess_consistency(cfg: Any, mode: str, epoch: int) -> bool:
    if not (mode in FR_MODES or mode in RR_MODES):
        return False
    enabled = bool(twc_get(cfg, ("unfolded", "train_postprocess_consistency"), True))
    if not enabled:
        return False
    default_start = int(twc_get(cfg, ("unfolded", "outage_ramp_start_epoch"), 1))
    start_epoch = int(twc_get(cfg, ("unfolded", "train_postprocess_start_epoch"), default_start))
    return epoch >= start_epoch


def _postprocess_output_for_mode(
    cfg: Any,
    batch: WidebandBatch,
    mode: str,
    w_raw: tf.Tensor,
    ...
    if lam0 is None:
        lam0 = initial_dual_from_batch(batch)
    lam0 = tf.cast(lam0, batch.delay_spread_ns.dtype)

    w_ref, lam1, _ = dual_calibration_refine(batch, w_raw, lam0, cfg, use_gate=True)
    w_feas, _ = repair_fs_feasibility(batch, w_ref, cfg, lam=lam1)
    if mode in RR_MODES:
        w_post, _ = recover_rate_with_feasible_mask(batch, w_ref, w_feas, cfg, lam=lam1)
    else:
        w_post = w_feas
    return w_post, lam1
    ...
    for epoch in range(1, epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            if _should_distill(cfg, epoch=epoch, global_step=global_step):
                teacher = _teacher_for_mode(cfg, batch, mode, epoch=epoch, total_epochs=epochs)
                teacher_runtimes.append(float(teacher.runtime_s))
            with tf.GradientTape() as tape:
                w_raw, _ = model(batch=batch, training=True)
                lambda_student = getattr(model, "last_lambda", None)
                w_eval = w_raw
                lambda_eval = lambda_student
                if _use_postprocess_consistency(cfg, mode, epoch):
                    w_post, lambda_post = _postprocess_output_for_mode(cfg, batch, mode, w_raw, lambda_student)
                    w_eval = w_raw + tf.stop_gradient(w_post - w_raw)
                    lambda_eval = lambda_post

                terms = _base_train_objective(cfg, batch, w_eval)
                loss = terms["loss"]
                ...
                raw_rate_anchor_weight = float(twc_get(cfg, ("unfolded", "raw_rate_anchor_weight"), 0.0))
                if raw_rate_anchor_weight > 0.0 and _use_postprocess_consistency(cfg, mode, epoch):
                    raw_terms = _base_train_objective(cfg, batch, w_raw)
                    loss = loss - tf.cast(raw_rate_anchor_weight, loss.dtype) * raw_terms["weighted_sum_rate"]
