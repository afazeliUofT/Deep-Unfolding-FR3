...
from .teacher import distillation_terms, solve_budget_dual_teacher
from .types import WidebandBatch
from .unfolded import train_unfolded_model, unfolded_inference, unfolded_mode
...
def _enabled_algorithms(cfg: Any) -> List[str]:
    ...
    if bool(twc_get(cfg, ("unfolded", "enabled"), True)):
        if unfolded_mode(cfg) in PD_MODES:
...
def _should_distill(cfg: Any, epoch: int, global_step: int) -> bool:
    if unfolded_mode(cfg) not in PD_MODES:
        return False
...
def _train(cfg: Any, pipe_dir: Path) -> Tuple[Optional[tf.keras.Model], pd.DataFrame]:
    ...
    distill_weight = float(twc_get(cfg, ("teacher", "distill", "weight"), 0.5))
    ...
    for epoch in range(1, epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            if _should_distill(cfg, epoch=epoch, global_step=global_step):
                teacher = solve_budget_dual_teacher(cfg, batch)
            ...
            with tf.GradientTape() as tape:
                ...
                if teacher is not None:
                    td = distillation_terms(batch, w, teacher, lambda_student=getattr(model, "last_lambda", None))
                    distill_loss_value = td["total"]
                    loss = loss + tf.cast(distill_weight, loss.dtype) * distill_loss_value
        ...
        for _ in range(val_batches):
            ...
            terms_val = _base_train_objective(cfg, batch_val, w_val)
