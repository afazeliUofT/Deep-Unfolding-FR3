        ...
        for algo_name, metric_path in sorted(legacy_metric_files.items()):
            ...
            for _, row in df.iterrows():
                ...
                legacy_rows.append(
                    {
                        "algorithm": algo_name,
                        "sweep_tag": str(row.get("sweep_variable", "legacy")),
                        "sweep_value": float(row.get("sweep_value", 0.0)),
                        ...
                        "fs_outage_prob_any": float(float(row.get("max_fs_violation_watt", 0.0)) > 0.0),
                        "worst_fs_excess_mean": float(row.get("max_fs_violation_watt", np.nan)),
                        "mean_positive_fs_excess": float(row.get("max_fs_interference_watt", np.nan)),
