...
def run_legacy_suite(config_path: str | Path, force_freeze_topology: str = "inherit") -> None:
    ...
    mode = str(force_freeze_topology).strip().lower()
    if mode in {"true", "false"}:
        common.append(f"experiment.freeze_topology={mode}")
    elif mode != "inherit":
        raise ValueError("force_freeze_topology must be one of: inherit, true, false")
...
def main() -> int:
    ...
    ap.add_argument(
        "--force-freeze-topology",
        default="inherit",
        choices=["inherit", "true", "false"],
        help="inherit keeps the config value; use true/false only for explicit protocol overrides.",
    )
    args = ap.parse_args()

    run_legacy_suite(REPO_ROOT / args.config, force_freeze_topology=args.force_freeze_topology)
    return 0
